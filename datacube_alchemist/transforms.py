...
class DeltaNBR_3band(Transformation):
    ...
    def compute(self, data) -> xr.Dataset:
        ...
        # Bare Soil Index (Rikimaru, Miyatake 2002) (BSI) = ((Swir2 + red) - (nir + Blue)) / ((Swir2 + red) + (nir + Blue))
        ...
        data["post_bsi"] = (
            (data.nbart_swir_2 + data.nbart_red )
            - (data.nbart_nir_1  + data.nbart_blue)
        ) / (
            (data.nbart_swir_2 + data.nbart_red)
            + (data.nbart_nir_1 + data.nbart_blue)
        )
        ...
        data["delta_bsi"] = data.delta_bsi * -1 #multiply by -1 to scale the same as other models
